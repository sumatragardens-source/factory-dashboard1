...
<header class="h-14 border-b border-border-card bg-bg-card flex items-center justify-between px-6 shrink-0">
	<div class="flex items-center gap-3">
		...
		<div class="group/nav relative">
			...
			<!-- Flyout nav panel -->
			<div
				class="absolute left-0 top-full pt-1 w-56 z-50 opacity-0 pointer-events-none group-hover/nav:opacity-100 group-hover/nav:pointer-events-auto transition-opacity duration-200 hidden group-hover/nav:flex flex-col"
			>
					...
					<nav class="flex-1 p-3 space-y-0.5 overflow-y-auto no-scrollbar">
						{#each NAV_ITEMS as item}
							{@const isActive =
								...
								(item.href !== '/operations' && page.url.pathname.startsWith(item.href))}
							<a
								href={item.href}
	...
	<div class="flex items-center gap-6">
		...
		<div class="flex items-center gap-4">
			<a
				href="/batches"
				class="bg-primary text-bg-dark px-4 py-1.5 rounded font-bold text-[10px] uppercase tracking-wider flex items-center gap-1.5"
			>
				<span class="material-symbols-outlined text-sm">add_box</span>
				Create New Batch
			</a>
			...
			<a
				href="/operations"
				class="p-1.5 text-text-muted rounded-full relative hover:text-text-primary transition-colors"
				aria-label="Notifications ({alertCounts?.total ?? 0} active alerts)"
			>
				<span class="material-symbols-outlined text-[20px]">notifications</span>
				{#if alertCounts && alertCounts.total > 0}
					<span
						class="absolute -top-0.5 -right-0.5 h-4 min-w-[16px] rounded-full {alertCounts.high > 0
							? 'bg-red-500'
							: 'bg-amber-500'} text-[9px] font-black text-white flex items-center justify-center px-1"
						>{alertCounts.total}</span
					>
				{/if}
			</a>

			<div
				class="h-8 w-8 rounded-full bg-border-card flex items-center justify-center text-text-secondary font-bold text-[10px] border border-border-subtle"
			>
				SG
			</div>
		</div>

	...
	<div class="fixed right-0 top-0 h-full w-[420px] z-50 bg-bg-card border-l border-border-card shadow-2xl flex flex-col overflow-hidden">
		...
		{:else if detail}
			...
			<div class="px-4 py-2 flex gap-3 text-[8px] text-text-muted border-b border-border-card/50">
				...
				{/if}
				{#if detail.batch.strain}
					<span><span class="text-text-muted/40">Strain:</span> {detail.batch.strain}</span>
				{/if}
				{#if detail.batch.supplier}
					<span><span class="text-text-muted/40">Sup:</span> {detail.batch.supplier}</span>
				{/if}
			</div>
			...
			<div class="px-4 py-2 border-b border-border-card/50">
				...
				<p class="text-[7px] text-text-muted/40 mt-1">{detail.stages.filter((s: any) => s.status === 'Finalized').length}/8 stages complete</p>
			</div>
			...
			<div class="flex-1 overflow-y-auto px-4 py-3">
				{#if activeTab === 'overview'}
					...
					<div class="grid grid-cols-2 gap-2 mb-3">
						<div class="bg-bg-input rounded p-2">
							...
							<p class="text-sm font-bold text-text-primary">{detail.stage4?.cumulative_yield_pct?.toFixed(1) ?? '—'}%</p>
						...
						<div class="bg-bg-input rounded p-2">
							...
							<p class="text-sm font-bold text-text-primary">{detail.stage2?.recovery_rate_pct?.toFixed(1) ?? '—'}%</p>
						...
						<div class="bg-bg-input rounded p-2">
							...
							<p class="text-sm font-bold text-text-primary">{detail.stage4?.final_product_weight_kg?.toFixed(2) ?? '—'} kg</p>
					...
					{#if detail.runAvgYield || detail.runAvgRecovery}
						...
						<div class="space-y-1.5">
							{#if detail.stage4?.cumulative_yield_pct && detail.runAvgYield}
								{@const diff = detail.stage4.cumulative_yield_pct - detail.runAvgYield}
								<div class="flex items-center gap-2">
									<span class="text-[8px] text-text-muted w-14">Yield</span>
									<div class="flex-1 h-1 rounded-full bg-border-card overflow-hidden">
										<div class="h-full rounded-full" style="width: {Math.min(100, (detail.stage4.cumulative_yield_pct / 10) * 100)}%; background: {diff >= 0 ? '#8BAA7C' : '#C4896A'};"></div>
									</div>
									<span class="text-[8px] font-medium" style="color: {diff >= 0 ? '#8BAA7C' : '#C4896A'};">{diff >= 0 ? '+' : ''}{diff.toFixed(1)}%</span>
								</div>
							{/if}
							{#if detail.stage2?.recovery_rate_pct && detail.runAvgRecovery}
								{@const diff = detail.stage2.recovery_rate_pct - detail.runAvgRecovery}
								<div class="flex items-center gap-2">
									<span class="text-[8px] text-text-muted w-14">Recovery</span>
									<div class="flex-1 h-1 rounded-full bg-border-card overflow-hidden">
										<div class="h-full rounded-full" style="width: {Math.min(100, detail.stage2.recovery_rate_pct)}%; background: {diff >= 0 ? '#8BAA7C' : '#C4896A'};"></div>
									</div>
									<span class="text-[8px] font-medium" style="color: {diff >= 0 ? '#8BAA7C' : '#C4896A'};">{diff >= 0 ? '+' : ''}{diff.toFixed(1)}%</span>
								</div>
							{/if}
					...
					{#if detail.stage2}
						<div class="grid grid-cols-2 gap-2 mb-3">
							<div class="bg-bg-input rounded p-2">
								...
								<p class="text-sm font-bold text-text-primary">{detail.stage2.ethanol_stock_used_l?.toFixed(0)} L</p>
							</div>
							<div class="bg-bg-input rounded p-2">
								<p class="text-[7px] text-text-muted/40 uppercase">Recovered</p>
								<p class="text-sm font-bold" style="color: #8BAA7C;">{detail.stage2.total_ethanol_recovered_l?.toFixed(0)} L</p>
							</div>
							<div class="bg-bg-input rounded p-2">
								<p class="text-[7px] text-text-muted/40 uppercase">Lost</p>
								<p class="text-sm font-bold" style="color: #C4896A;">{detail.stage2.total_ethanol_loss_l?.toFixed(0)} L</p>
							</div>
							<div class="bg-bg-input rounded p-2">
						...
						{#if detail.stage3}
							...
							<div class="flex gap-3 text-[8px] text-text-muted">
								<span>Used: {detail.stage3.limonene_volume_l}L</span>
								<span>Recovered: {detail.stage3.limonene_recovered_l}L</span>
								<span style="color: #C4896A;">Lost: {detail.stage3.limonene_loss_l}L</span>
							</div>
						{/if}
					{:else}
				...
				{:else if activeTab === 'quality'}
					...
					<div class="space-y-0 mb-3">
						...
						{#if detail.stage1?.powder_weight_kg}
							...
							<div class="flex items-center gap-1.5">
								...
								<span class="text-[8px] font-medium text-text-secondary">{detail.stage1.powder_weight_kg} kg</span>
							</div>
						{/if}
						{#if detail.stage2?.extract_weight_kg}
							<div class="flex items-center gap-1.5 py-px">
								<div class="w-1 flex justify-center"><div class="w-px h-2" style="background: rgba(139, 170, 124, 0.25);"></div></div>
							</div>
							<div class="flex items-center gap-1.5">
								...
								<span class="text-[8px] font-medium text-text-secondary">{detail.stage2.extract_weight_kg} kg</span>
							</div>
						{/if}
						{#if detail.stage4?.final_product_weight_kg}
							<div class="flex items-center gap-1.5 py-px">
								<div class="w-1 flex justify-center"><div class="w-px h-2" style="background: rgba(139, 170, 124, 0.25);"></div></div>
							</div>
							<div class="flex items-center gap-1.5">
								...
								<span class="text-[8px] font-medium text-text-secondary">{detail.stage4.final_product_weight_kg} kg</span>
							</div>
						{/if}
					</div>

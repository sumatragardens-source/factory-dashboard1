...
export function getApprovalsByBatch(batchId: number): Approval[] {
	const db = getDb();
	return db.prepare('SELECT * FROM approvals WHERE batch_id = ? ORDER BY requested_at DESC').all(batchId) as Approval[];
}

export function createApproval(data: { batch_id: number; approval_type: string; requested_by: string }): number {
	const db = getDb();
	const result = db
		.prepare(`INSERT INTO approvals (batch_id, approval_type, requested_by) VALUES (?, ?, ?)`)

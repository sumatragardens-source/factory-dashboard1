...
import type {
	Stage1Record, Stage2Record, Stage3Record, Stage4Record,
	Stage3Filtration, Stage4Distillation, Stage5AcidBase,
	Stage6BackextPrecip, Stage7DryingFinal, ReactorLoad
...
export function getReactorLoads(batchId: number): ReactorLoad[] {
	const db = getDb();
	return db.prepare('SELECT * FROM reactor_loads WHERE batch_id = ? ORDER BY load_number').all(batchId) as ReactorLoad[];
}

// ── Stage 3: Filtration & Washing ─────────────────────────────────────────

const STAGE3_FILT_COLS = [
	'filter_press_cycles', 'centrifuge_batches', 'screw_press_used', 'de_used',
	'wash_volume_L', 'cake_weight_kg', 'filtrate_volume_L', 'filtrate_clarity',
	'operator_name', 'notes'
] as const;

export function getStage3Filtration(batchId: number): Stage3Filtration | undefined {
	const db = getDb();
	return db.prepare('SELECT * FROM stage3_filtration WHERE batch_id = ?').get(batchId) as
		| Stage3Filtration
		| undefined;
}
...
const STAGE4_DIST_COLS = [
	'distillation_temp_C', 'vacuum_mbar', 'bath_temp_C', 'distill_time_min',
	'recovered_etoh_L', 'recovered_abv_pct', 'recovery_pct',
	'crude_aqueous_vol_L', 'crude_aqueous_wt_kg', 'residual_alcohol_pct',
	'operator_name', 'notes'
] as const;
...
const STAGE5_COLS = [
	...
	'settling_min', 'organic_phase_mL', 'aqueous_waste_L', 'emulsion_events',
	'operator_name', 'notes'
] as const;

export function getStage5AcidBase(batchId: number): Stage5AcidBase | undefined {
	const db = getDb();
	return db.prepare('SELECT * FROM stage5_acid_base WHERE batch_id = ?').get(batchId) as
		| Stage5AcidBase
		| undefined;
}
...
const STAGE6_COLS = [
	'acetic_conc', 'acetic_water_vol_L', 'acetic_pure_vol_L', 'backext_cycles',
	'backext_settle_min', 'dlimo_recovered_L', 'dlimo_lost_L', 'dlimo_loss_pct',
	'acidic_aq_vol_L', 'acidic_ph', 'k2co3_added_g', 'precip_ph', 'precip_temp_C',
	'wet_precipitate_g', 'wash_vol_mL', 'wash_cycles', 'centrifuge_rpm',
	'supernatant_vol_L', 'operator_name', 'notes'
] as const;
...
const STAGE7_COLS = [
	'dry_method', 'dry_temp_C', 'dry_time_h', 'final_product_g',
	'final_moisture_pct', 'overall_yield_pct', 'batch_duration_hr',
	'storage_location', 'operator_name', 'notes'
] as const;

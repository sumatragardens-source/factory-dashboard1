...
export function createSchema(db: Database.Database): void {
	db.exec(`
		...
		CREATE TABLE IF NOT EXISTS stage2_records (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			batch_id INTEGER NOT NULL UNIQUE REFERENCES batches(id),
			ethanol_stock_grade_pct REAL,
			target_ethanol_pct REAL DEFAULT 70,
			ethanol_stock_used_l REAL,
			water_added_l REAL,
			ethanol_70_volume_l REAL,
			settle_time_min REAL,
			bag_filter_input_l REAL,
			bag_filter_output_l REAL,
			centrifuge_input_l REAL,
			centrifuge_output_l REAL,
			screw_press_input_l REAL,
			screw_press_output_l REAL,
			total_ethanol_70_to_rotovap_l REAL,
			total_ethanol_distilled_l REAL,
			water_mother_liquid_l REAL,
			total_ethanol_recovered_l REAL,
			total_ethanol_loss_l REAL,
		...
		CREATE TABLE IF NOT EXISTS stage2_reactors (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			batch_id INTEGER NOT NULL REFERENCES batches(id),
			reactor_number INTEGER NOT NULL CHECK(reactor_number BETWEEN 1 AND 3),
			...
			ethanol_70_volume_l REAL,
			solvent_ratio REAL,
			UNIQUE(batch_id, reactor_number)
		);

		CREATE TABLE IF NOT EXISTS stage2_rotovap_days (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			batch_id INTEGER NOT NULL REFERENCES batches(id),
			rotovap_number INTEGER NOT NULL CHECK(rotovap_number BETWEEN 1 AND 2),
			machine_id INTEGER REFERENCES machines(id),
			day_number INTEGER NOT NULL,
			water_bath_temp_c REAL,
			vacuum_mbar REAL,
			chiller_temp_c REAL,
			rpm REAL,
			run_time_hours REAL,
			ethanol_recovered_l REAL,
			recovery_per_hour_l REAL,
			UNIQUE(batch_id, rotovap_number, day_number)
		);

...
export const load: PageServerLoad = () => {
	...
	`
		...
		for (const [batchId, info] of batchMap) {
			...
			const costPerKg =
				info.finalProductG && info.finalProductG > 0
			...
			const vsTarget =
				costPerKg != null ? Number((((costPerKg - TARGETS.costPerKg) / TARGETS.costPerKg) * 100).toFixed(1)) : null;
		...
		const batchesWithDeviation = batchSummaries.map((b) => ({
			...b,
			vsAvg:
				b.costPerKg != null && avgCostPerKg > 0
					? Number((((b.costPerKg - avgCostPerKg) / avgCostPerKg) * 100).toFixed(1))
					: null,
			status:
				b.costPerKg == null
					? 'N/A'
					: b.vsTarget != null && b.vsTarget <= -5
						? 'High Eff'
						: b.vsTarget != null && b.vsTarget > 5
							? 'Critical'
							: 'On Target'

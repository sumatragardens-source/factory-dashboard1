<script lang="ts">
	...
	const costDelta = data.overallCostPerKg > 0
...
{#if data.batchCount > 0}
	...
	<div class="flex gap-2 mb-4 overflow-x-auto">
		...
		<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
			...
			<span class="text-[10px] font-bold {costDelta <= 0 ? 'text-[#bef264]' : 'text-[#ef4444]'}">{costDelta <= 0 ? '' : '+'}{costDelta}% vs target</span>
		</div>
		<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
		...
		<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
			...
			<div class="text-xl font-black {data.margin >= 0 ? 'text-[#bef264]' : 'text-[#ef4444]'}">{fmt(data.margin)}</div>
		</div>
	</div>
	...
	<div class="grid grid-cols-12 gap-4">
		...
		<div class="col-span-8 flex flex-col gap-4">
				...
				<div class="space-y-4">
					{#each data.stageAccumulation as stage}
						<div>
							...
							<div class="h-7 bg-[#0d0d0d]/50 rounded-sm overflow-hidden" style="width: {(stage.total / maxStageTotal) * 100}%">
			...
			<div class="bg-[#161616] border border-[#1e1e1e] rounded-lg overflow-hidden">
				<h2 class="text-[10px] font-bold text-[#666666] uppercase tracking-widest px-4 pt-4 pb-2">Batch Cost Table</h2>
					...
					{#each data.paretoDrivers.slice(0, 5) as driver, i}
						<div class="bg-[#161616] rounded">
							...
							<div class="h-2 w-full bg-[#0d0d0d] rounded overflow-hidden">
								<div class="h-full rounded {i < 3 ? 'bg-[#ec5b13]' : 'bg-[#4b5563]'}" style="width: {driver.pct}%"></div>
							</div>

	...
	{#if data.batches && data.batches.length > 0}
		<div class="flex items-center justify-between mb-6">
			...
			<button
				...
				class="bg-primary text-text-primary px-4 py-2 rounded font-bold text-xs uppercase tracking-tighter flex items-center gap-2 hover:brightness-105 transition-all"
			>
		...
		{#if showNewForm}
			<form method="POST" action="?/create" class="bg-bg-card border border-primary/20 rounded p-6 mb-6">
				...
				<div class="grid grid-cols-4 gap-4">
					<div class="space-y-1">
						...
						<select
							name="supplier"
							required
							class="w-full bg-bg-card-hover border-border-card rounded-lg text-sm focus:ring-primary"
						>
				...
				<div class="mt-4 flex justify-end">
					<button
						type="submit"
						class="bg-primary text-text-primary px-6 py-2 rounded font-bold text-xs uppercase tracking-widest hover:brightness-105 transition-all flex items-center gap-2"
					>
			...
			{#each statuses as st}
				<button
					...
					{statusFilter === st
						...
						: 'bg-bg-card text-text-muted hover:bg-bg-card-hover border border-border-card'}"
					onclick={() => (statusFilter = st)}
				>
					{st}
					{st === 'All' ? `(${data.batches.length})` : `(${data.batches.filter((b) => b.status === st).length})`}
				</button>
			{/each}
		</div>
			...
			<table class="w-full text-left">
				...
				<tbody>
					{#each filteredBatches as batch}
						<tr
							class="border-b border-border-subtle hover:bg-bg-card-hover transition-colors cursor-pointer"
							role="link"
							tabindex="0"
							onclick={() => {
						...
						>
							<td class="px-4 py-3">
								<a href="/batches/{batch.id}" class="text-sm font-bold text-text-primary hover:text-primary"
							...
							<td class="px-4 py-3 text-xs text-text-muted font-mono"
								>{new Date(batch.created_at).toLocaleDateString()}</td
							>
						</tr>
					{/each}
				</tbody>

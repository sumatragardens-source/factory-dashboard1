<script lang="ts">
	...
	import {
		calculateLeafDryMass,
		calculateGrindThroughput,
		calculateDryingLoss,
		calculateOverallDryYield
	} from '$lib/calculations/yield';
	...
	const grindThroughput = $derived(
		s1PowderOutput > 0 && s1Runtime > 0 ? calculateGrindThroughput(s1PowderOutput, s1Runtime) : null
	);
	...
	const dryingLoss = $derived(
		s4WetPrecip > 0 && s4FinalProduct > 0 ? calculateDryingLoss(s4WetPrecip, s4FinalProduct) : null
	);
	const overallDryYield = $derived(() => {
		const leafDry =
			data.stage1?.net_leaf_kg && data.stage1?.moisture_pct != null
				? calculateLeafDryMass(data.stage1.net_leaf_kg, data.stage1.moisture_pct)
				: 0;
	...
	const disabledClass =
		'w-full bg-bg-input/50 border-border-card rounded-lg text-sm text-text-muted cursor-not-allowed';
	const inputClass = $derived(
		isFinalized
			? disabledClass
			: 'w-full bg-bg-input border-border-card rounded-lg text-sm text-text-primary focus:ring-primary'
	);
	const readonlyClass =
		'w-full bg-primary/5 border-none rounded-lg text-sm font-bold text-text-primary focus:ring-primary';
	const highlightClass = $derived(
		isFinalized
			? disabledClass
			: 'w-full bg-primary/10 border-primary/30 border-2 rounded-lg text-sm font-black text-text-primary focus:ring-primary'
	);
	const labelClass = 'text-[10px] font-bold uppercase text-text-muted';
	const reqLabelClass =
		"text-[10px] font-bold uppercase text-text-muted after:content-['*'] after:text-red-400 after:ml-0.5";
...
<div class="flex h-full overflow-hidden">
			...
			{#each STAGE_KEYS as sn, i}
				...
				<a
					...
					class="flex items-center gap-3 px-3 py-3 rounded-lg text-sm transition-colors
						...
						: 'text-text-secondary hover:bg-bg-card-hover cursor-pointer'}"
				>
	...
	<main class="flex-1 flex flex-col h-full overflow-hidden">
		<form method="POST" use:enhance class="flex-1 flex flex-col h-full overflow-hidden">
			<header
				class="h-16 border-b border-border-card px-8 flex items-center justify-between bg-bg-card/80 backdrop-blur-md shrink-0"
			>
				<div class="flex items-center gap-4">
					...
					{#if isFinalized}
						<span
							class="ml-2 px-2 py-0.5 rounded bg-primary/20 text-primary text-[10px] font-black uppercase tracking-widest flex items-center gap-1"
						>
				...
				{#if !isFinalized}
					<div class="flex gap-3">
						<button
							type="submit"
							formaction="?/save"
							class="px-4 py-2 bg-primary/20 text-text-primary rounded text-sm font-bold flex items-center gap-2 hover:bg-primary/30 transition-colors"
						>
						...
						{#each unacked as alert}
							...
							>
								...
								<form method="POST" action="?/acknowledge" use:enhance>
									...
									<button type="submit" class="text-xs font-bold px-2 py-1 rounded bg-white/10 hover:bg-white/20"
										>Acknowledge</button
									>
								</form>
							</div>
								...
								<div class="grid grid-cols-3 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											step="0.01"
											name="gross_leaf_kg"
											value={data.stage1?.gross_leaf_kg ?? ''}
									...
									<div class="space-y-1">
										...
										<input
											...
											step="0.01"
											name="container_kg"
											value={data.stage1?.container_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
									...
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="moisture_pct"
											value={data.stage1?.moisture_pct ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="screen_microns"
											value={data.stage1?.screen_microns ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
								...
								<div class="grid grid-cols-3 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.01"
											name="powder_output_kg"
											value={data.stage1?.powder_output_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.01"
											name="dust_loss_kg"
											value={data.stage1?.dust_loss_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
									...
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="mass_balance_err_pct"
											value={data.stage1?.mass_balance_err_pct ?? ''}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.01"
											name="throughput_kg_hr"
											value={data.stage1?.throughput_kg_hr ?? ''}
										/>
									</div>
								</div>
							</section>
								...
								<div class="grid grid-cols-2 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="text"
											name="operator_name"
											value={data.stage1?.operator_name ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
							...
							<div class="mb-4 bg-primary/5 border border-primary/15 rounded-xl p-4">
								...
								{#if data.stage1 && data.stages.find((s) => s.stage_number === 1)?.status === 'Finalized'}
									<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
										<div>
											<p class="text-[10px] text-text-muted">Net Leaf</p>
											<p class="text-sm font-bold text-text-primary">{data.stage1.net_leaf_kg} kg</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Powder Output</p>
											<p class="text-sm font-bold text-text-primary">{data.stage1.powder_output_kg} kg</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Moisture</p>
											<p class="text-sm font-bold text-text-primary">{data.stage1.moisture_pct}%</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Dust Loss</p>
											<p class="text-sm font-bold text-text-primary">{data.stage1.dust_loss_kg} kg</p>
										</div>
									</div>
								{:else}
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="etoh_purity_pct"
											value={data.stage2?.etoh_purity_pct ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="extract_temp_C"
											value={data.stage2?.extract_temp_C ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="extract_time_min"
											value={data.stage2?.extract_time_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="extract_cycles"
											value={data.stage2?.extract_cycles ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="agitation_rpm"
											value={data.stage2?.agitation_rpm ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="stir_time_min"
											value={data.stage2?.stir_time_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="settle_time_min"
											value={data.stage2?.settle_time_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="filtrate_vol_L"
											value={data.stage2?.filtrate_vol_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.01"
											name="spent_cake_kg"
											value={data.stage2?.spent_cake_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
								</div>
								...
								<div class="grid grid-cols-3 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="etoh_recovered_L"
											value={data.stage2?.etoh_recovered_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="etoh_recovery_pct"
											value={data.stage2?.etoh_recovery_pct ?? ''}
										/>
									</div>
									<div class="space-y-1">
									...
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="distill_time_min"
											value={data.stage2?.distill_time_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="rotovap_vacuum_mbar"
											value={data.stage2?.rotovap_vacuum_mbar ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="rotovap_bath_C"
											value={data.stage2?.rotovap_bath_C ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="crude_aqueous_vol_L"
											value={data.stage2?.crude_aqueous_vol_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.01"
											name="crude_extract_wt_kg"
											value={data.stage2?.crude_extract_wt_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
								</div>
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="water_temp_C"
											value={data.stage2?.water_temp_C ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="water_time_min"
											value={data.stage2?.water_time_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="water_cycles"
											value={data.stage2?.water_cycles ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="water_filtrate_L"
											value={data.stage2?.water_filtrate_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.01"
											name="water_spent_cake_kg"
											value={data.stage2?.water_spent_cake_kg ?? ''}
											disabled={isFinalized}
										/>
									</div>
								</div>
								...
								<div class="grid grid-cols-2 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="text"
											name="operator_name"
											value={data.stage2?.operator_name ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
						...
						{:else if data.stageNumber === 3}
							...
							<div class="mb-4 bg-primary/5 border border-primary/15 rounded-xl p-4">
								...
								{#if data.stage2 && data.stages.find((s) => s.stage_number === 2)?.status === 'Finalized'}
									<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
										<div>
											<p class="text-[10px] text-text-muted">Crude Extract</p>
											<p class="text-sm font-bold text-text-primary">{data.stage2.crude_extract_wt_kg} kg</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">EtOH Issued</p>
											<p class="text-sm font-bold text-text-primary">{data.stage2.etoh_vol_L} L</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">EtOH Recovered</p>
											<p class="text-sm font-bold text-text-primary">{data.stage2.etoh_recovered_L} L</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Recovery %</p>
											<p class="text-sm font-bold text-text-primary">{data.stage2.etoh_recovery_pct}%</p>
										</div>
									</div>
								{:else}
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="naoh_added_g"
											value={data.stage3?.naoh_added_g ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="partition_cycles"
											value={data.stage3?.partition_cycles ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="settling_min"
											value={data.stage3?.settling_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="organic_phase_mL"
											value={data.stage3?.organic_phase_mL ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="aqueous_waste_L"
											value={data.stage3?.aqueous_waste_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
								</div>
								...
								<div class="grid grid-cols-3 gap-6">
									...
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="acetic_water_vol_L"
											value={data.stage3?.acetic_water_vol_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="acetic_pure_vol_L"
											value={data.stage3?.acetic_pure_vol_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											name="backext_cycles"
											value={data.stage3?.backext_cycles ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											class={inputClass}
											type="number"
											name="backext_settle_min"
											value={data.stage3?.backext_settle_min ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="dlimo_recovered_L"
											value={data.stage3?.dlimo_recovered_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="dlimo_lost_L"
											value={data.stage3?.dlimo_lost_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="dlimo_loss_pct"
											value={data.stage3?.dlimo_loss_pct ?? ''}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="acidic_aq_vol_L"
											value={data.stage3?.acidic_aq_vol_L ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
								...
								<div class="grid grid-cols-2 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="text"
											name="operator_name"
											value={data.stage3?.operator_name ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
						...
						{:else if data.stageNumber === 4}
							...
							<div class="mb-4 bg-primary/5 border border-primary/15 rounded-xl p-4">
								...
								{#if data.stage3 && data.stages.find((s) => s.stage_number === 3)?.status === 'Finalized'}
									<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
										<div>
											<p class="text-[10px] text-text-muted">D-Limo Issued</p>
											<p class="text-sm font-bold text-text-primary">{data.stage3.dlimo_vol_L} L</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">D-Limo Recovered</p>
											<p class="text-sm font-bold text-text-primary">{data.stage3.dlimo_recovered_L} L</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Acidic Aq Vol</p>
											<p class="text-sm font-bold text-text-primary">{data.stage3.acidic_aq_vol_L} L</p>
										</div>
										<div>
											<p class="text-[10px] text-text-muted">Organic Phase</p>
											<p class="text-sm font-bold text-text-primary">{data.stage3.organic_phase_mL} mL</p>
										</div>
									</div>
								{:else}
								...
								<div class="grid grid-cols-3 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="k2co3_added_g"
											value={data.stage4?.k2co3_added_g ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
									...
									<div class="space-y-1">
										...
										<input
											...
											step="0.1"
											name="precip_temp_C"
											value={data.stage4?.precip_temp_C ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="wet_precipitate_g"
											value={data.stage4?.wet_precipitate_g ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
								...
								<div class="grid grid-cols-3 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="final_product_g"
											value={data.stage4?.final_product_g ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="final_moisture_pct"
											value={data.stage4?.final_moisture_pct ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.01"
											name="overall_yield_pct"
											value={data.stage4?.overall_yield_pct ?? ''}
										/>
									</div>
									<div class="space-y-1">
										...
										<input
											...
											type="number"
											step="0.1"
											name="batch_duration_hr"
											value={data.stage4?.batch_duration_hr ?? ''}
											disabled={isFinalized}
										/>
									</div>
								</div>
								...
								<div class="grid grid-cols-2 gap-6">
									<div class="space-y-1">
										...
										<input
											...
											type="text"
											name="operator_name"
											value={data.stage4?.operator_name ?? ''}
											disabled={isFinalized}
										/>
									</div>
									<div class="space-y-1">
					...
					<div class="col-span-4 flex flex-col gap-6">
						<div class="sticky top-0">
							{#if data.stageNumber === 1 && data.stage1}
								<div
									class="bg-bg-card text-text-primary p-8 rounded-xl border border-primary/40 shadow-xl relative overflow-hidden"
								>
									<div class="absolute -right-10 -top-10 w-40 h-40 bg-primary/20 rounded-full blur-3xl"></div>
									<h3 class="text-xs font-black uppercase tracking-widest text-primary mb-8">
									...
									<div class="space-y-6 relative z-10">
										<div class="flex justify-between items-end border-b border-border-card pb-4">
											<div>
												...
												<p class="text-2xl font-black">
													{data.stage1.net_leaf_kg?.toFixed(2)}
													<span class="text-xs font-normal text-text-muted">kg</span>
												</p>
											</div>
										...
										<div class="flex justify-between items-end border-b border-border-card pb-4">
											<div>
												...
												<p class="text-2xl font-black">
													{data.stage1.powder_output_kg?.toFixed(2)}
													<span class="text-xs font-normal text-text-muted">kg</span>
												</p>
											</div>
										...
										<div class="flex justify-between items-end bg-primary/10 -mx-4 px-4 py-4 rounded-lg">
											<div>
												...
												<p class="text-3xl font-black text-primary">
													{data.stage1.dust_loss_kg?.toFixed(2) ?? '0.00'} <span class="text-sm font-normal">kg</span>
												</p>
											</div>
										...
										<div class="pt-4 border-t border-border-card">
											<div class="flex justify-between mb-2">
												...
												<span
													class="text-[10px] font-bold {(data.stage1.mass_balance_err_pct ?? 0) <= 2
														? 'text-primary'
														: 'text-amber-400'}">{data.stage1.mass_balance_err_pct ?? 0}%</span
												>
											</div>
											<div class="w-full bg-border-card h-2 rounded-full overflow-hidden">
												<div
													class="{(data.stage1.mass_balance_err_pct ?? 0) <= 2
														? 'bg-primary'
														: 'bg-amber-400'} h-full rounded-full"
													style="width: {Math.min((data.stage1.mass_balance_err_pct ?? 0) * 20, 100)}%"
												></div>
											</div>
											<p
												class="text-[9px] {(data.stage1.mass_balance_err_pct ?? 0) <= 2
													? 'text-primary'
													: 'text-amber-400'} mt-1"
											>
												{(data.stage1.mass_balance_err_pct ?? 0) <= 2 ? 'Within tolerance' : 'Above 2% threshold'}
											</p>
										</div>
									</div>
									{#if leafDryMass}
										<div class="pt-4 border-t border-border-card">
											<div class="flex justify-between items-center">
												...
												<span class="text-sm font-black text-text-primary"
													>{leafDryMass} <span class="text-xs font-normal text-text-muted">kg</span></span
												>
											</div>
										</div>
									{/if}
									{#if grindThroughput}
										<div class="pt-4 border-t border-border-card">
											<div class="flex justify-between items-center">
												...
												<span class="text-sm font-black text-text-primary"
													>{grindThroughput} <span class="text-xs font-normal text-text-muted">kg/hr</span></span
												>
											</div>
										</div>
									{/if}
									...
									<div class="space-y-6 relative z-10">
										...
										<div class="flex justify-between items-end border-b border-border-card pb-4">
											<div>
												...
												<p class="text-2xl font-black">
													{data.stage2.etoh_lost_L ?? '—'} <span class="text-xs font-normal text-text-muted">L</span>
												</p>
											</div>
										...
										<div class="flex justify-between items-end bg-primary/10 -mx-4 px-4 py-4 rounded-lg">
											<div>
												...
												<p class="text-3xl font-black text-primary">
													{data.stage2.crude_extract_wt_kg ?? '—'} <span class="text-sm font-normal">kg</span>
												</p>
											</div>
										</div>
										<div class="flex justify-between items-end border-b border-border-card pb-4">
											<div>
												...
												<p class="text-2xl font-black">
													{data.stage2.filtrate_vol_L ?? '—'} <span class="text-xs font-normal text-text-muted">L</span>
												</p>
											</div>
										</div>
										<div class="flex justify-between items-end border-b border-border-card pb-4">
											<div>
												...
												<p class="text-2xl font-black">
													{data.stage2.spent_cake_kg ?? '—'} <span class="text-xs font-normal text-text-muted">kg</span>
												</p>
											</div>
										</div>
										{#if ssRatio}
											<div class="flex justify-between items-end border-b border-border-card pb-4">
												<div>
													...
													<p class="text-2xl font-black">
														{ssRatio.toFixed(1)} <span class="text-xs font-normal text-text-muted">: 1</span>
													</p>
												</div>
										...
										<div class="space-y-3">
											...
											<div class="flex justify-between items-center text-xs">
												<span class="flex items-center gap-1.5"
													><span class="w-2 h-2 rounded-full bg-primary"></span> Recovered</span
												>
												<span class="font-bold text-primary"
													>{data.stage2.etoh_recovered_L} L ({etohRecoveryPct.toFixed(1)}%)</span
												>
											</div>
											<div class="flex justify-between items-center text-xs">
												<span class="flex items-center gap-1.5"
													><span class="w-2 h-2 rounded-full bg-amber-500"></span> Lost</span
												>
												<span class="font-bold text-amber-400"
													>{etohLost.toFixed(1)} L ({(100 - etohRecoveryPct).toFixed(1)}%)</span
												>
											</div>
										</div>
									</div>
								...
								{#if crudeYieldPct != null}
									<div class="bg-bg-card p-6 rounded-xl border border-border-card shadow-sm mt-4">
										...
										<p class="text-[10px] text-text-muted mt-1">
											crude yield from {data.stage1?.powder_output_kg} kg powder
										</p>
									</div>
								{/if}
							{:else if data.stageNumber === 3 && data.stage3}
									...
									{#if data.stage3.dlimo_loss_pct != null}
										<div class="mt-4 pt-4 border-t border-border-card">
											<div class="flex justify-between mb-2">
												...
												<span
													class="text-[10px] font-bold {(data.stage3.dlimo_loss_pct ?? 0) <= 10
														? 'text-primary'
														: 'text-amber-400'}">{data.stage3.dlimo_loss_pct}%</span
												>
											</div>
											<div class="w-full bg-border-card h-2 rounded-full overflow-hidden">
												<div
													class="{(data.stage3.dlimo_loss_pct ?? 0) <= 10
														? 'bg-primary'
														: 'bg-amber-400'} h-full rounded-full"
													style="width: {Math.min(data.stage3.dlimo_loss_pct ?? 0, 100)}%"
												></div>
											</div>
										</div>
									{/if}
								...
								{#if data.batch?.leaf_input_kg}
									<div class="bg-bg-card p-6 rounded-xl border border-border-card shadow-sm mt-4">
										<h4 class="text-xs font-black uppercase tracking-widest text-text-muted mb-4">
											Cumulative Progress
										</h4>
										<div class="space-y-3">
											<div>
												...
												<div>
													<div class="flex justify-between text-[11px] mb-1">
														...
														<span class="font-bold text-text-primary"
															>{data.stage1.powder_output_kg} kg {#if powderRetentionPct}({powderRetentionPct.toFixed(
																	1
																)}%){/if}</span
														>
													</div>
													<div class="w-full bg-border-card h-2 rounded-full overflow-hidden">
														<div
															...
															style="width: {powderRetentionPct ?? 0}%"
														></div>
													</div>
												</div>
											{/if}
											{#if data.stage2?.crude_extract_wt_kg}
												<div>
													<div class="flex justify-between text-[11px] mb-1">
														...
														<span class="font-bold text-text-primary"
															>{data.stage2.crude_extract_wt_kg} kg {#if crudeYieldPct}({crudeYieldPct.toFixed(1)}% of
																powder){/if}</span
														>
													</div>
													<div class="w-full bg-border-card h-2 rounded-full overflow-hidden">
														<div
															...
															style="width: {data.batch.leaf_input_kg > 0
																? (data.stage2.crude_extract_wt_kg / data.batch.leaf_input_kg) * 100
																: 0}%"
														></div>
													</div>
												</div>
							...
							{:else if data.stageNumber === 4 && data.stage4}
								<div class="bg-bg-card p-6 rounded-xl border border-border-card shadow-sm">
									...
									<div class="space-y-4">
										...
										<div class="bg-primary/10 p-4 rounded-lg border border-primary/30">
											...
											<p class="text-3xl font-black text-primary">
												{data.stage4.overall_yield_pct ?? '—'} <span class="text-sm font-normal">%</span>
											</p>
										</div>
										{#if data.stage4.final_moisture_pct != null}
											<div>
												...
												<p class="text-xl font-black text-text-primary">
													{data.stage4.final_moisture_pct} <span class="text-xs font-normal text-text-muted">%</span>
												</p>
											</div>
										{/if}
										{#if dryingLoss != null}
											<div>
												...
												<p class="text-xl font-black text-text-primary">
													{dryingLoss} <span class="text-xs font-normal text-text-muted">%</span>
												</p>
											</div>
										{/if}
										{#if overallDryYield() != null}
											<div class="bg-amber-900/10 p-4 rounded-lg border border-amber-500/30">
												...
												<p class="text-3xl font-black text-amber-400">
													{overallDryYield()} <span class="text-sm font-normal">%</span>
												</p>
											</div>
										{/if}
										{#if data.stage4.batch_duration_hr}
											<div>
												...
												<p class="text-xl font-black text-text-primary">
													{data.stage4.batch_duration_hr} <span class="text-xs font-normal text-text-muted">hr</span>
												</p>
											</div>
										{/if}
									</div>
										...
										<div class="space-y-2">
											...
											{#if data.stage1?.powder_output_kg}
												<div class="flex items-center gap-2 text-[11px] pl-2">
													...
													{#if powderRetentionPct}<span class="text-[10px] text-primary"
															>({powderRetentionPct.toFixed(1)}%)</span
														>{/if}
												</div>
											{/if}
											{#if data.stage2?.crude_extract_wt_kg}
												<div class="flex items-center gap-2 text-[11px] pl-4">
													...
													{#if crudeYieldPct}<span class="text-[10px] text-primary">({crudeYieldPct.toFixed(1)}%)</span
														>{/if}
												</div>
											{/if}
											{#if data.stage4?.final_product_g}
												<div class="flex items-center gap-2 text-[11px] pl-6">
													...
													<span class="ml-auto font-black text-primary"
														>{(data.stage4.final_product_g / 1000).toFixed(3)} kg</span
													>
													{#if data.stage4.overall_yield_pct}<span class="text-[10px] text-primary"
															>({data.stage4.overall_yield_pct}%)</span
														>{/if}
												</div>
											{/if}
										</div>
								...
								{#if data.stage4.wet_precipitate_g && data.stage4.final_product_g}
									<div class="bg-bg-card p-6 rounded-xl border border-border-card shadow-sm mt-4">
										<h4 class="text-[10px] font-black uppercase tracking-widest text-text-muted mb-4">
											Detailed Metrics
										</h4>
										<div class="space-y-3">
											<div class="flex justify-between items-center text-xs">
												...
												<span class="font-bold text-text-primary"
													>{(data.stage4.wet_precipitate_g - data.stage4.final_product_g).toFixed(1)} g</span
												>
											</div>
											{#if data.totalCost && data.stage4.final_product_g}
												<div class="flex justify-between items-center text-xs border-t border-border-card pt-2 mt-2">
													...
													<span class="font-black text-primary"
														>{fmt(data.totalCost / (data.stage4.final_product_g / 1000))}</span
													>
												</div>
											{/if}
										</div>
			...
			<footer class="h-14 border-t border-border-card px-8 flex items-center justify-between bg-bg-card shrink-0">
				<div class="flex items-center gap-4 text-xs font-bold text-text-muted">
					<span class="flex items-center gap-1"
						><span class="w-2 h-2 bg-primary rounded-full"></span> SYSTEM ONLINE</span
					>
				</div>
				<div class="flex gap-4">
					<a href="/batches/{data.batch.id}" class="text-sm font-bold text-text-muted hover:text-text-primary"
						>Back to Batch</a
					>
					{#if isFinalized}
					...
					{:else}
						<button
							type="submit"
							formaction="?/finalize"
							class="bg-primary text-text-primary px-8 py-2 rounded-lg text-sm font-black uppercase tracking-widest hover:brightness-110 transition-all"
							>Finalize Stage</button
						>
					{/if}
				</div>
			</footer>

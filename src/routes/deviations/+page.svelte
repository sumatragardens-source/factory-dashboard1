		...
		<div class="grid grid-cols-4 gap-4 mb-6">
			...
			<div class="bg-bg-card border border-border-card p-4 rounded">
				...
				<p class="text-2xl font-black text-text-primary">
					{data.totalCount > 0
						? Math.round(((data.byStatus.Resolved + data.byStatus.Closed) / data.totalCount) * 100)
						: 0}%
				</p>
			</div>
		</div>
				...
				<div class="flex gap-2">
					{#each ['All', 'Open', 'Resolved', 'Closed'] as filter}
						<button
							disabled
							title="Coming soon"
							class="text-[10px] font-bold uppercase px-3 py-1 rounded {filter === 'All'
			...
			<table class="w-full text-left">
				<thead>
					<tr class="border-b border-border-card">
						...
						<th class="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-text-muted text-center"
							>Expected</th
						>
						<th class="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-text-muted text-center"
							>Actual</th
				...
				<tbody>
					{#each data.deviations as dev}
						<tr class="border-b border-border-subtle hover:bg-bg-card-hover/50">
							...
							<td
								class="px-6 py-3 text-sm font-bold text-center font-mono {dev.severity === 'Critical'
									? 'text-red-400'
									: 'text-text-primary'}">{dev.actual_value ?? '—'}</td
							>
							<td class="px-6 py-3 text-center">
								<span class="text-[10px] font-bold uppercase px-2 py-1 rounded {severityColors[dev.severity]}"
									>{dev.severity}</span
								>
							</td>
							<td class="px-6 py-3 text-center">
								<span class="text-[10px] font-bold uppercase px-2 py-1 rounded {statusColors[dev.status]}"
									>{dev.status}</span
								>
							</td>
							<td class="px-6 py-3">
						...
						{#each data.deviations.filter((d) => d.root_cause) as dev}
							<div class="border-l-2 border-primary pl-3">
								<p class="text-xs font-bold text-text-primary">
									{dev.batch_number} · {getProcessStageName(dev.stage_number)}
								</p>

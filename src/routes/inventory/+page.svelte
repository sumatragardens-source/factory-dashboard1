	...
	{:else}
		<div class="flex items-center justify-between mb-6">
			...
			<div class="flex gap-3">
				<button
					disabled
					title="Coming soon"
					class="bg-slate-900 text-white px-4 py-2 rounded font-bold text-xs uppercase tracking-tighter flex items-center gap-2 opacity-50 cursor-not-allowed"
				>
				...
				<button
					disabled
					title="Coming soon"
					class="bg-primary/10 text-text-primary border border-primary/30 px-4 py-2 rounded font-bold text-xs uppercase tracking-tighter flex items-center gap-2 opacity-50 cursor-not-allowed"
				>
		...
		<div class="bg-bg-card border border-border-card rounded overflow-hidden mb-8">
			<div class="px-6 py-3 border-b border-border-card flex justify-between items-center">
				...
				<span class="text-[10px] text-primary bg-primary/10 px-2 py-0.5 rounded font-bold"
					>{data.materials.length} Tracked Materials</span
				>
			</div>
				...
				<tbody>
					{#each data.materials as mat}
						...
						<tr class="border-b border-border-subtle hover:bg-bg-card-hover">
							...
							<td class="px-6 py-4">
								{#if isBelowThreshold}
									<span class="text-sm font-bold italic text-red-400"
										>{mat.on_hand_qty} {mat.unit} <span class="text-red-500">▲</span></span
									>
								{:else}
							...
							<td class="px-6 py-4">
								<span
									class="text-sm {isBelowThreshold
										? 'bg-red-900/30 text-red-400 px-2 py-0.5 rounded font-bold'
										: 'bg-bg-input text-text-secondary px-2 py-0.5 rounded'}">{mat.reorder_threshold} {mat.unit}</span
								>
							</td>
							<td class="px-6 py-4">
								<span class="text-xs text-primary bg-primary/10 px-2 py-0.5 rounded font-medium"
									>{resolveStageRelevance(mat.stage_relevance ?? '')}</span
								>
							</td>
						</tr>
					{/each}
		...
		<div class="bg-bg-card border border-border-card rounded overflow-hidden">
			<div class="px-6 py-3 border-b border-border-card flex justify-between items-center">
				...
				<button
					disabled
					title="Coming soon"
					class="text-[10px] text-primary font-bold uppercase opacity-50 cursor-not-allowed">View Full Log</button
				>
			</div>
			<table class="w-full text-left">
				<thead>
				...
				<tbody>
					{#each data.recentMovements as mv}
						...
						<tr class="border-b border-border-subtle">
							...
							<td class="px-6 py-3">
								<span
									class="text-xs font-bold {mv.movement_type === 'Recovered'
										? 'bg-primary/20 text-primary'
										: mv.movement_type === 'Issued'
											? 'bg-slate-900 text-white'
											: 'bg-bg-input text-text-secondary'} px-2 py-0.5 rounded">{mv.movement_type}</span
								>
							</td>
							<td class="px-6 py-3 text-xs text-text-secondary"
								>{mv.material_name} {mv.batch_number ? `(Batch ${mv.batch_number})` : ''}</td
							>
							<td class="px-6 py-3 text-xs font-bold {isPositive ? 'text-primary' : 'text-text-primary'}"
								>{isPositive ? '+' : '-'}{mv.quantity} {mv.unit}</td
							>
						</tr>
					{/each}
				</tbody>

		...
		<div class="col-span-3">
			...
			<div class="space-y-2">
				{#each data.labResults as result}
					<button
						class="w-full text-left p-3 rounded border transition-colors {selectedId === result.id ? 'border-primary bg-primary/5' : 'border-border-card bg-bg-card hover:border-border-card'}"
								...
								<table class="w-full text-left">
									<thead>
										<tr class="border-b border-border-card">
											...
											<th class="py-3 text-[10px] font-black uppercase tracking-widest text-text-muted text-center">Concentration (%)</th>
											<th class="py-3 text-[10px] font-black uppercase tracking-widest text-text-muted text-center">LOQ (%)</th>
											<th class="py-3 text-[10px] font-black uppercase tracking-widest text-text-muted text-right">Status</th>
										</tr>
									</thead>
									<tbody>
										{#each alkaloids as alk}
											<tr class="border-b border-border-subtle">
												...
												<td class="py-3 text-sm font-bold text-text-primary text-center font-mono">{alk.pct !== null ? `${alk.pct}%` : '< 0.02%'}</td>
							...
							<div class="bg-bg-card border border-border-card rounded p-6">
								...
								<div class="space-y-2">
									...
									<div class="flex justify-between">
										...
										<span class="text-xs font-bold text-text-primary">{selected.test_date ? new Date(selected.test_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</span>
									</div>
								...
								{#if selected.status === 'Completed'}
									<div class="flex items-start gap-3">
										...
										<div>
											...
											<p class="text-[10px] text-text-muted">{selected.updated_at ? new Date(selected.updated_at).toLocaleString() : '—'}</p>
										</div>
									</div>
								{/if}
								<div class="flex items-start gap-3">
									...
									<div>
										...
										<p class="text-[10px] text-text-muted">{selected.test_date ? new Date(selected.test_date).toLocaleString() : '—'}</p>
									</div>
								</div>
								<div class="flex items-start gap-3">
									...
									<div>
										...
										<p class="text-[10px] text-text-muted">{selected.created_at ? new Date(selected.created_at).toLocaleString() : '—'}</p>
									</div>
								</div>
								<div class="flex items-start gap-3">

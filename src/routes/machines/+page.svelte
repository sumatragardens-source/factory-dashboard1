				...
				<div class="bg-bg-card border border-border-card rounded p-6">
						...
						<div class="flex items-center gap-3">
							<div class="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
								<span class="material-symbols-outlined text-primary"
									>{machineIcons[machine.machine_type] ?? 'settings'}</span
								>
							</div>
							<div>
								<h3 class="text-sm font-bold text-text-primary">{machine.name}</h3>
								<p class="text-[10px] uppercase tracking-wider text-text-muted font-bold">
									{resolveStageRelevance(machine.stage_relevance ?? '')}
								</p>
							</div>
						</div>
						<span
							class="flex items-center gap-1.5 text-[10px] font-bold uppercase px-2 py-1 rounded {statusBadge[
								machine.status
							]}"
						>
					...
					<div class="flex gap-2">
						<button
							disabled
							title="Coming soon"
							class="flex-1 border border-border-card text-text-secondary py-2 rounded text-xs font-bold uppercase opacity-50 cursor-not-allowed"
							>Maintenance</button
						>
						<button
							disabled
							title="Coming soon"
							class="flex-1 bg-primary text-text-primary py-2 rounded text-xs font-bold uppercase opacity-50 cursor-not-allowed"
						>

...
interface LeafTotals {
	total_leaf_kg: number;
}
interface CakeData {
	total_spent_cake: number;
}
...
export const load: PageServerLoad = () => {
	...
	`
		...
		const ssrRatio =
			leafTotals.total_leaf_kg > 0 ? Number((ethanol.total_issued / leafTotals.total_leaf_kg).toFixed(2)) : 0;
	...
	`
		...
		const adjEvaporation = Number((totalLost * 0.3).toFixed(1));
	...
	`
		...
		const batchPerformance = batchMatrix.map((row) => ({
			...
			vsAvg: row.recovery_pct != null ? Number((row.recovery_pct - avgRecovery).toFixed(1)) : null,
			status:

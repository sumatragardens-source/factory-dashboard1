...
{#if data.batchCount > 0}
	...
	<div class="flex gap-2 mb-4 overflow-x-auto">
		<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
			...
			<span class="text-[10px] font-bold {recoveryDelta >= 0 ? 'text-[#bef264]' : 'text-[#ef4444]'}">{recoveryDelta >= 0 ? '+' : ''}{recoveryDelta}% vs avg</span>
		</div>
		<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
	...
	<div class="grid grid-cols-12 gap-4">
		...
		<div class="col-span-8 flex flex-col gap-4">
			...
			<div class="bg-[#161616] border border-[#1e1e1e] rounded-lg overflow-hidden">
				<h2 class="text-[10px] font-bold text-[#666666] uppercase tracking-widest px-4 pt-4 pb-2">Batch Performance Matrix</h2>
						...
						<tbody class="divide-y divide-[#1e1e1e]/50 font-mono">
							{#each data.batchPerformance.slice(0, 15) as row}
								<tr class="hover:bg-white/5">
									...
									<td class="px-4 py-2 text-right font-bold {row.recovery_pct >= 86 ? 'text-[#bef264]' : row.recovery_pct >= 83 ? 'text-[#f59e0b]' : 'text-[#ef4444]'}">{row.recovery_pct}%</td>
								</tr>
							{/each}
						</tbody>
					</table>
		...
		<div class="col-span-4">
			<div class="bg-[#161616] border border-[#1e1e1e] rounded-lg p-4">
				<h2 class="text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-3">Loss Sources: Worst Batches</h2>
				<div class="space-y-3">
					{#each worstBatches as batch}
						<div class="flex items-center gap-3">
							...
							<div class="flex-1 h-3 bg-[#0d0d0d] rounded overflow-hidden">
								<div class="h-full bg-gradient-to-r from-[#ef4444] to-[#ec5b13] rounded" style="width: {(batch.lost / maxBatchLoss) * 100}%"></div>
							</div>

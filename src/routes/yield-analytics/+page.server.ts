...
export const load: PageServerLoad = () => {
	try {
		...
		const overallYieldPct = cascade.leaf > 0
			? Number((cascade.finalProduct / cascade.leaf * 100).toFixed(2))
			: 0;
		const extractRatePct = cascade.leaf > 0
			? Number((cascade.crude / cascade.leaf * 100).toFixed(2))
			: 0;
		...
		const batchTable = batchYields.map(b => ({
			...
			vsAvg: b.overall_yield_pct != null
				? Number((b.overall_yield_pct - avgYield).toFixed(2))
				: null,
		}));
		...
		const qualityCorrelation = db.prepare(`
		...
		return {
			...
			maxStageLoss: 1,
			qualityCorrelation: [],
			qualitySpecs: { purity: null, purityTarget: 90, mitragynine: null, mitragynineTarget: 1.0, moisture: null, moistureTarget: 6 },
			solventTrend: [],
			cycleTimes: [],
			alkaloids: [],
		};

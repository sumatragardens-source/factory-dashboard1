...
<div class="p-6">
	<div class="grid grid-cols-12 gap-6">
		...
		<div class="col-span-3 flex flex-col gap-4">
			...
			<div class="bg-white border border-slate-200 rounded p-4">
				<p class="text-[10px] uppercase tracking-widest text-slate-500 font-black mb-4">Alkaloid Composition</p>
					...
					{#each alkaloids as alk}
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2">
								...
								<span class="text-xs text-slate-600">{alk.name}</span>
							</div>
							<span class="text-xs font-bold text-slate-900">{alk.pct}%</span>
						</div>
					{/each}
				</div>
			</div>
		...
		<div class="col-span-6 flex flex-col gap-6">
			...
			<div class="bg-white border border-slate-200 rounded p-6">
				<div class="flex items-center justify-between mb-4">
					<h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500">Yield Comparison by Batch</h3>
				...
				<div class="flex items-end gap-3 h-48">
					{#each data.batchYields as batch}
						<div class="flex-1 flex flex-col items-center gap-1">
							...
							<span class="text-[10px] text-slate-500 font-mono">{batch.batch_number.replace('SG-2026-', 'B-')}</span>
						</div>
					{/each}
				</div>
			</div>
			...
			<div class="bg-white border border-slate-200 rounded p-6">
				<h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Solvent Performance Trend</h3>
				<p class="text-[10px] text-slate-400 mb-4">Recovery Rate (%)</p>
			...
			<div class="bg-white border border-slate-200 rounded p-6">
				<h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Stage Yield Analysis</h3>
				<div class="space-y-4">
					{#each data.stageYields as sy}
						<div>
							<div class="flex justify-between mb-1">
								<span class="text-sm text-slate-700">{sy.stage}</span>
								<span class="text-sm font-bold text-slate-900">{sy.pct}%</span>
							</div>
		...
		<div class="col-span-3 flex flex-col gap-6">
			...
			<div class="bg-white border border-slate-200 rounded p-4">
				<div class="flex items-center justify-between mb-4">
					<h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500">Cost Per KG Trend</h3>
					<span class="text-sm font-black text-slate-900">${data.latestCostPerKg.toLocaleString()}</span>
				</div>
				<div class="h-24 flex items-end gap-1">
					{#each data.costPerKgData as cpk}
						<div class="flex-1 bg-slate-200 rounded-t" style="height: {Math.max((cpk.costPerKg / (Math.max(...data.costPerKgData.map((c: any) => c.costPerKg)) || 1)) * 100, 5)}%"></div>
					{/each}
				</div>
			...
			<!-- Batch Cycle Time -->
			<div class="bg-white border border-slate-200 rounded p-4">
				<h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Batch Cycle Time (HRS)</h3>
				<div class="space-y-3">
					{#each data.cycleTimes as ct}
						<div class="flex items-center gap-2">
							<span class="text-xs text-slate-600 w-12 shrink-0 font-mono">{ct.batch_number.replace('SG-2026-', 'B-')}</span>
							...
							<span class="text-xs font-bold text-slate-900 w-10 text-right">{ct.hours}h</span>
						</div>
					{/each}
				</div>
			</div>

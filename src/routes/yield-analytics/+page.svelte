	...
	{:else}
			...
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-[10px] text-[#ec5b13] font-bold tracking-[0.2em] uppercase">
						{data.batchCount} Batches Analyzed
					</p>
				</div>
			</div>
		</div>
		...
		<div class="flex gap-2 mb-4 overflow-x-auto">
			...
			<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
				...
				<div class="text-xl font-black text-[#bef264]">
					{bestBatch ? bestBatch.overall_yield_pct.toFixed(2) + '%' : '—'}
				</div>
				{#if bestBatch}
			...
			<div class="flex-1 min-w-[140px] bg-gray-900 border border-white/10 rounded-lg p-3">
				...
				<div class="text-xl font-black text-[#ef4444]">
					{worstBatch ? worstBatch.overall_yield_pct.toFixed(2) + '%' : '—'}
				</div>
				{#if worstBatch}
			...
			<div class="col-span-8 flex flex-col gap-4">
				...
				<div class="bg-[#161616] border border-[#1e1e1e] rounded-lg p-4">
					...
					<div class="flex items-center justify-between gap-2">
						...
						<div class="flex flex-col items-center min-w-[40px]">
							...
							<span class="text-[9px] bg-[#ef4444]/20 text-[#ef4444] px-1 rounded font-bold"
								>-{data.cascadeLosses.leafToPowder}kg</span
							>
						</div>
						<!-- Powder -->
						<div class="flex-1 bg-[#0d0d0d] border border-[#2a2a2a] p-3 rounded text-center">
						...
						<div class="flex flex-col items-center min-w-[40px]">
							<span class="material-symbols-outlined text-[#4b5563]">arrow_forward</span>
							<span class="text-[9px] bg-[#ef4444]/20 text-[#ef4444] px-1 rounded font-bold"
								>-{data.cascadeLosses.powderToCrude}kg</span
							>
						</div>
						<!-- Crude -->
						...
						<div class="flex flex-col items-center min-w-[40px]">
							<span class="material-symbols-outlined text-[#4b5563]">arrow_forward</span>
							<span class="text-[9px] bg-[#ef4444]/20 text-[#ef4444] px-1 rounded font-bold"
								>-{data.cascadeLosses.crudeToWet}kg</span
							>
						</div>
						<!-- Final -->
				...
				<div class="bg-[#161616] border border-[#1e1e1e] rounded-lg p-4">
					<h2 class="text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-3">
						Yield Comparison (Recent 20 Batches)
					</h2>
						...
						{#each chartBatches as batch}
							<div class="flex items-center gap-2">
								...
								<div class="flex-1 h-3 bg-[#0d0d0d] rounded relative overflow-hidden">
									<div
										class="h-full rounded {(batch.overall_yield_pct ?? 0) >= data.avgYield
											? 'bg-[#bef264]'
											: 'bg-[#ef4444]'}"
										style="width: {((batch.overall_yield_pct ?? 0) / maxYield) * 100}%"
									></div>
								</div>
								<span class="text-[9px] font-mono font-bold text-white w-12 text-right"
									>{batch.overall_yield_pct != null ? batch.overall_yield_pct.toFixed(2) + '%' : '—'}</span
								>
							</div>
						{/each}
						...
						{#each data.stageYields as sy}
							<div>
								...
								<div class="w-full bg-[#0d0d0d] rounded-full h-2.5">
									<div
										class="h-full rounded-full {sy.pct >= 80
											? 'bg-[#bef264]'
											: sy.pct >= 50
												? 'bg-[#ec5b13]'
												: 'bg-[#ef4444]'}"
					...
					<div class="space-y-4">
						...
						<div>
							...
							<p class="text-lg font-black">
								{data.qualitySpecs.purity != null ? data.qualitySpecs.purity + '%' : 'No Data'}
							</p>
							<div class="mt-1 h-1.5 w-full bg-[#0d0d0d] rounded-full overflow-hidden">
								<div
									class="h-full rounded-full {data.qualitySpecs.purity != null && data.qualitySpecs.purity >= 90
										? 'bg-[#bef264]'
										: 'bg-[#ef4444]'}"
									style="width: {data.qualitySpecs.purity != null ? Math.min(data.qualitySpecs.purity, 100) : 0}%"
								></div>
							</div>
						...
						<div>
							...
							<p class="text-lg font-black">
								{data.qualitySpecs.mitragynine != null ? data.qualitySpecs.mitragynine + '%' : 'No Data'}
							</p>
							<div class="mt-1 h-1.5 w-full bg-[#0d0d0d] rounded-full overflow-hidden">
								<div
									class="h-full rounded-full {data.qualitySpecs.mitragynine != null &&
									data.qualitySpecs.mitragynine >= 1.0
										? 'bg-[#bef264]'
										: 'bg-[#ef4444]'}"
									style="width: {data.qualitySpecs.mitragynine != null
										? Math.min(data.qualitySpecs.mitragynine * 10, 100)
										: 0}%"
								></div>
							</div>
						...
						<div>
							...
							<p class="text-lg font-black">
								{data.qualitySpecs.moisture != null ? data.qualitySpecs.moisture + '%' : 'No Data'}
							</p>
							<div class="mt-1 h-1.5 w-full bg-[#0d0d0d] rounded-full overflow-hidden">
								<div
									class="h-full rounded-full {data.qualitySpecs.moisture != null && data.qualitySpecs.moisture <= 6
										? 'bg-[#bef264]'
										: 'bg-[#ef4444]'}"
									style="width: {data.qualitySpecs.moisture != null
										? Math.min((data.qualitySpecs.moisture / 12) * 100, 100)
										: 0}%"
								></div>
							</div>
